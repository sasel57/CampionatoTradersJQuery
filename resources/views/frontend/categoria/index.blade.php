        ...
        <div class="col-md-12">
            @can('categorium_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.categoria.create') }}">
                            ...
                            <tbody>
                                @foreach($categoria as $key => $categorium)
                                    <tr data-entry-id="{{ $categorium->id }}">
                                        ...
                                        <td>
                                            @can('categorium_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.categoria.show', $categorium->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('categorium_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.categoria.edit', $categorium->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('categorium_delete')
                                                <form action="{{ route('frontend.categoria.destroy', $categorium->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('categorium_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.categoria.massDestroy') }}",
